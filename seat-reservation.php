    ...
    <script>
        ...
        proceedBtn.addEventListener('click', () => {
            ...
            const seatsData = selectedSeatNumbers.map(seatId => {
                // If getSeatPOV exists (from seat-pov.php), use it to determine quality
                let tier = 'Standard';
                let price = 250;
                
                if (typeof getSeatPOV === 'function') {
                    const pov = getSeatPOV(seatId);
                    if (pov) {
                        tier = pov.qualityLabel;
                        // Define prices based on quality labels
                        switch (tier) {
                            case 'Prime':    price = 450; break;
                            case 'Great':    price = 350; break;
                            case 'Good':     price = 280; break;
                            case 'Too Close':price = 220; break;
                            case 'Far Back': price = 220; break;
                            case 'Fair':     price = 250; break;
                            case 'Angled':   price = 180; break;
                            case 'Poor':     price = 150; break;
